...
class EP {
   ...
   private:
	...
	TVectorD totmultp[nbin][neta];
	TVectorD totmultn[nbin][neta];
	TVectorD totmultc[nbin][neta];
	TVectorD totpt[nbin];
...
EP::calcEP()
{
   using namespace std;
	int mult;
		...
		hEPPhin[xbin][ieta]->Fill(EPPhim[ieta]);
...
EP::beginJob()
{
	...
	for(int ibin=0; ibin<nbin; ibin++){
		...
		hEPPhip[ibin][ieta] = new TH1D("",Form("Event Plane Angle Distribution bin %d #eta>%.f",ieta,etap[ieta]),400,-4,4);
		hEPPhin[ibin][ieta] = new TH1D("",Form("Event Plane Angle Distribution bin %d #eta<%.f",ieta,etan[ieta]),400,-4,4);
		hEPPhic[ibin][ieta] = new TH1D("",Form("Event Plane Angle Distribution bin %d |#eta|<%.2f",ieta,etac),400,-4,4);
		}
        }
}
...
EP::endJobEP(TString outstr) 
{
	...
	for(int ieta=0;ieta<neta;ieta++){
		...
		totmultp[ibin][ieta].Write(Form("totmultp_%d_%d",ibin,ieta));
		totmultn[ibin][ieta].Write(Form("totmultn_%d_%d",ibin,ieta));
		totmultc[ibin][ieta].Write(Form("totmultc_%d_%d",ibin,ieta));
		hEPPhip[ibin][ieta]->SetName(Form("hEPPhip_%d_%d",ibin,ieta));
		hEPPhin[ibin][ieta]->SetName(Form("hEPPhin_%d_%d",ibin,ieta));
