...
//global
  ...
  float bbcde_eta[nbbcs];
        ...
        for(int iparticle = 0;iparticle < n; iparticle++){
          if(fabs(eta[iparticle])<0.35 && pt[iparticle]>0.2 && pt[iparticle]< 5.0){
              track_pt[ntrk] = pt[iparticle];
              track_phi[ntrk] = phi[iparticle];
              track_eta[ntrk] = eta[iparticle];
              ntrk++;
          }
            if(fabs(eta[iparticle])>3 && fabs(eta[iparticle])<4 && pt[iparticle]>0.05){
      int iarm = 0;
      if (eta[iparticle] > 0) iarm = 1;
	if(iarm==0){
	  bbcau_phi[nbbcau] = phi[iparticle]; 
	  bbcau_eta[nbbcau] = eta[iparticle]; 
	  nbbcau++;
	}
	else{
          bbcde_phi[nbbcde] = phi[iparticle];
          bbcde_eta[nbbcde] = eta[iparticle];
          nbbcde++;
	}
         if(iarm==0)   nch++;
    }
   ...
   // int chbin[centbin+1] = {0,1,2,3,4,5,7,10,14,18,40}; //both side
    int chbin[centbin+1] = {0,1,2,3,4,5,7,10,12,16,40};
    hnch->Fill(nch);
