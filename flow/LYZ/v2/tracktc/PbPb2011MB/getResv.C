...
void getResv(int ispt){
	...
	for(int ibin=0;ibin<nbin;ibin++){
        ...
        for(int ifile=0; ifile<nFileAll; ifile++){
	        ...
	        if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/tracktc/Anaveta_Prod_%d.root",ifile));
	        else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/tracktc/Anaveta_Prod2_%d.root",ifile));
            }
