...
void dothird(){
        ...
        TString name;
        for(int i=start;i<end;i++){
                if(SumorProd=="Sum")       name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/tracktc/Anaveta_Prod_%d.root",i);
                else            name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/tracktc/Anaveta_Prod2_%d.root",i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(0);
