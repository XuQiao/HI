...
void getResvsub(int ispt=1){
	...
	for(int itheta=0;itheta<ntheta;itheta++)
        ...
        if(ispt==1){             nvv = nptv;       binv = ptbinv;}
	...
	if(SumorProd=="Sum")TFile *fout = new TFile("mergedv_Prod_coarse_eta_sub.root","Recreate");
	...
	for(int ibin=0;ibin<nbin;ibin++){
        for(int ifile=0; ifile<nFileAll; ifile++){
            if(ispt==1){
        if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/tracknormcpt03to6/trackzvtxl/%s/Anav_Prod_%d.root",mdir.c_str(),ifile));
        else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/tracknormcpt03to6/trackzvtxl/%s/Anav_Prod2_%d.root",mdir.c_str(),ifile));
            }
            else if(ispt==-1){
        if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/tracknormcpt03to6/trackzvtxl/coarsebins/%s/Anaveta_Prod_%d.root",mdir.c_str(),ifile));
        else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/tracknormcpt03to6/trackzvtxl/coarsebins/%s/Anaveta_Prod2_%d.root",mdir.c_str(),ifile));
            }
            else{
        if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/tracknormcpt03to6/trackzvtxl/%s/Anaveta_Prod_%d.root",mdir.c_str(),ifile));
        else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/tracknormcpt03to6/trackzvtxl/%s/Anaveta_Prod2_%d.root",mdir.c_str(),ifile));
            }
		...
		Nevent0[ibin] += (*Nevent_t)[ibin];
			...
			for(int itheta=0;itheta<ntheta;itheta++){
				...
				for(int ivbin=0;ivbin<nvv;ivbin++){
					...
					if(ifile<223) dN[ibin][itheta][ivbin]+=TComplex((*dNRe[ibin][itheta])[ivbin],(*dNIm[ibin][itheta])[ivbin]);
