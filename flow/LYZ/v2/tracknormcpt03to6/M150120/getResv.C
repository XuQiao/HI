...
void getResv(int ispt){
			...
			for(int itheta=0;itheta<ntheta;itheta++){
				...
				for(int ivbin=0;ivbin<nvv;ivbin++){
					...
					if(ifile<188) dN[ibin][itheta][ivbin]+=TComplex((*dNRe[ibin][itheta])[ivbin],(*dNIm[ibin][itheta])[ivbin]);
                                        dN[ibin][itheta][nvv-ivbin-1]+=TComplex((*dNRe[ibin][itheta])[ivbin],(*dNIm[ibin][itheta])[ivbin]);
                                }
                                }
			}
		}
	...
	hpteff->Scale(1.0/NbinX);
		...
		avgmultall[ibin]=1.0*totmultall[ibin]/Nevent[ibin];
			...
			for(int itheta=0;itheta<ntheta;itheta++){
				...
				v[ibin][itheta][ivbin]=(*V[ibin])[itheta]*avgmult[ibin][ivbin]*TMath::BesselJ1(j01)/Besselj01(mm)*Res.Re();
	...
	if(SumorProd=="Sum")TFile *fout = new TFile("mergedv_Prod_eta.root","Recreate");
        ...
        if(ispt==-1){
	if(SumorProd=="Sum")TFile *fout = new TFile("mergedv_Prod_coarse_eta.root","Recreate");
	else TFile *fout = new TFile("mergedv_Prod2_coarse_eta.root","Recreate");
        }
