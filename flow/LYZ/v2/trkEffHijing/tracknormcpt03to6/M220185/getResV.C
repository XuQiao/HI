...
//------------------------Deal with the number and get the output stored in txt files and root files
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/trkEffHijing/tracknormcpt03to6/%s/AnaV_Sum_%d.root",dir.c_str(),ifile));
		else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/trkEffHijing/tracknormcpt03to6/%s/AnaV_Prod_%d.root",dir.c_str(),ifile));
