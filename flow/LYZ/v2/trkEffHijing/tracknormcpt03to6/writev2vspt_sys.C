...
ofstream fout(Form("pPbsysN%d.dat",nsamples));
int xbin=0;
...
for(int i=0;i<ntotbin;i++){
	...
	TFile *fProdlc = TFile::Open(Form("../tracktc/tracknormcpt03to6/M%d%d/mergedv_Prod2_sub.root",trkpointmax[i],trkpointmin[i]));
	...
	int npt = vecDavgpt_Prod->GetNrows();
        ...
        fout<<"avgpt\tv2\tv2 err stat.\tv2 err per.\tv2 tight\tv2 loose\t3<|z|<15\t|z|<3"<<endl;
        for(int iptbin=0;iptbin<npt;iptbin++){
            fout<<avgpt_Prod[iptbin]<<"\t";
            fout<<v2_Prod[iptbin]<<"\t";
            fout<<v2err_Prod[iptbin]<<"\t";
            fout<<v2err_Prod[iptbin]/v2_Prod[iptbin]<<"\t";
            fout<<fabs(v2_Prodtc[iptbin]/v2_Prod[iptbin]-1)<<"\t";
            fout<<fabs(v2_Prodlc[iptbin]/v2_Prod[iptbin]-1)<<"\t";
            fout<<fabs(v2_Prodzl[iptbin]/v2_Prod[iptbin]-1)<<"\t";
            fout<<fabs(v2_Prodzs[iptbin]/v2_Prod[iptbin]-1)<<"\t";
            fout<<endl;
        }
