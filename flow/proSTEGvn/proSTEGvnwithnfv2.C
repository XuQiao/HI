 ...
 //	V2vsPt->SetParameters(4.81159,1.80783,3.69272,3.11889,0.931485);	//Real data V~0.05
  ...
  for(int i=0; i<tot_num; i++){ 
    ...
    for(int j=0;j<b_npgsame;j++){
      b_ptg[n] = b_ptg[k];
      b_etag[n] = rnd->Gaus(b_etag[k], 0.1);
      if(fabs(b_etag[n])>4) continue;
      b_phig[n] = rnd->Gaus(b_phig[k], 0.1);
      n++;
    }
    for(int j=0;j<b_npgback;j++){
      ...
      b_etag[n] = rnd->Gaus(-b_etag[k], 0.1);
      if(fabs(b_etag[n])>4) continue;
      b_phig[n] = rnd->Gaus(TMath::Pi()+b_phig[k], 0.1);
