...
//    else if(trigtype.Contains("mb"))  name = "../filelistmb.dat";
    ...
    for(int i=start;i<end;i++){
        cout << i << endl;
        //Ridgecntbbc *pl = new Ridgecntbbc(readline(Form("%s",name.Data()),i), Form("output/Ana%s_%d.root",trigtype.Data(),i));
        Ridgecntbbc *pl = new Ridgecntbbc(readline(Form("%s",name.Data()),i), readline(Form("%s",name1.Data()),i), readline(Form("%s",name2.Data()),i), Form("/scratch/xuq7/phenix/PP/Ridgecntbbc/output/AnaMWG%s_%d.root",trigtype.Data(),i));
        pl->Init();
        pl->process_event();
        pl->End();
        delete pl;
