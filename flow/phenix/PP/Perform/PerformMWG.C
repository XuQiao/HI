...
PerformMWG::PerformMWG(TString input, TString input1, TString input2, TString output):
...
//fvtx
  for(int iarm=0; iarm<2; iarm++){
    ...
    sprintf(name,"DCAxy2dis_%d",iarm); DCAxy2dis[iarm] = new TH2D(name,name, 100, -10000, 10000, 100, -10000, 10000);
...
//correlate
  ...
  sprintf(name,"hnfvtxtrkpc1hits"); hnfvtxtrkpc1hits = new TH2F(name, name, 100, 0, 100, 100, 0, 100);
  ...
  sprintf(name,"hrunntrack_arm0_pos"); hrunntrack[0] = new TH2F(name,name,432008-422070+21,422070-10.5,432008+10.5,50,0,50);
  sprintf(name,"hrunntrack_arm0_neg"); hrunntrack[1] = new TH2F(name,name,432008-422070+21,422070-10.5,432008+10.5,50,0,50);
  sprintf(name,"hrunntrack_arm1_pos"); hrunntrack[2] = new TH2F(name,name,432008-422070+21,422070-10.5,432008+10.5,50,0,50);
  sprintf(name,"hrunntrack_arm1_neg"); hrunntrack[3] = new TH2F(name,name,432008-422070+21,422070-10.5,432008+10.5,50,0,50);
  sprintf(name,"hrunnfvtxtrack"); hrunnfvtxtrack = new TH2F(name,name,432008-422070+21,422070-10.5,432008+10.5,50,0,50);

  return 0;
}

int PerformMWG::process_event()
{
...
//cnt track
 ...
 //     bool emc_matching = fabs(emcsdphi)<2.0 && fabs(emcsdz)<2.0;
	...
	if(arm[itrk]==1 && charge[itrk] < 0)	tofwdphidz[1]->Fill(tofdphi[itrk],tofdz[itrk]);
      ...
      if(pt>0.2&&pt<5.0){
    if(charge[itrk] > 0 && arm[itrk] == 0)     ntrkdiff[0]++; 
    if(charge[itrk] < 0 && arm[itrk] == 0)     ntrkdiff[1]++;
    if(charge[itrk] > 0 && arm[itrk] == 1)     ntrkdiff[2]++;
...
//FVTX
  ...
  for (int ij=0; ij<nfvtxtrack; ij++) {
	...
	sigma_dcay= (DCA_y-0.0684611)/0.135455;
     ...
     //bool dcacut = true;
      if(fvtx_phi<10 && fvtx_phi > -10 && fabs(fvtx_eta)<3.5 && fabs(fvtx_eta) > 2.5 && dcacut && nhits>=3){
...
//  hntracknmpc->Fill(ntrk,nmpc_south+nmpc_north);
  ...
  hnfvtxtrkpc1hits->Fill(nfvtxtrk,npc1hits);
...
// }
   hrunnfvtxtrack -> Fill(run,nfvtxtrk);
}
return 0;
}
