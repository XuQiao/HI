...
gStyle->SetOptStat(kFALSE);
 TFile *f = new TFile("mergedFull.root","ReadOnly");
const int ncav = 26;
TCanvas *c1[ncav];
...
//Tracks
  TH2F *hcntetaphi = (TH2F*)f->Get("hcntetaphi");
c1[0]->cd();
SetTitle(*hcntetaphi,"cnt #eta","cnt #phi","");
hcntetaphi->Draw("colz");
c1[0]->Print("fig/mb/hcntetaphi.png");

  TH1F *hcntpt = (TH1F*)f->Get("hcntpt");
c1[1]->cd();
c1[1]->SetLogy();
hcntpt->Rebin(4);
SetTitle(*hcntpt,"cnt p_{T}","# of tracks","");
SetStyle(*hcntpt,1.2,1,20,0,0);
hcntpt->Draw("P");
c1[1]->Print("fig/mb/hcntpt.png");

TH2F* pc3dphidz = (TH2F*)f->Get("pc3dphidz_arm0_pos_0");
TH1F* pc3dphi = (TH1F*)pc3dphidz->ProjectionX("pc3dphi",0,-1); 
TH1F* pc3dz = (TH1F*)pc3dphidz->ProjectionY("pc3dz",0,-1); 
c1[2]->cd();
SetTitle(*pc3dphi,"pc3 d#eta","# of tracks","");
SetStyle(*pc3dphi,1.2,1,20,0,0);
pc3dphi->Draw("P");
c1[2]->Print("fig/mb/pc3dphi.png");

c1[3]->cd();
SetStyle(*pc3dz,1.2,1,20,0,0);
pc3dz->Rebin(4);
SetTitle(*pc3dz,"pc3 dz","# of tracks","");
pc3dz->Draw("P");
c1[3]->Print("fig/mb/pc3dz.png");
  
TH2F* hntracknmpc = (TH2F*)f->Get("hntracknmpc");
TH1F* hntrack = (TH1F*)hntracknmpc->ProjectionX("hntrack",0,-1);
c1[4]->cd();
c1[4]->SetLogy();
SetTitle(*hntrack,"ntrack","# of events","");
//hntrack->Scale(1./hntrack->Integral());
SetStyle(*hntrack,1.2,1,20,0,0);
//SetRange(*hntrack,0,1e-11,20,10);
SetRange(*hntrack,0,1,15,1e10);
hntrack->Draw();
c1[4]->Print("fig/mb/ntrack.png");
/*
//tof
...
c1[21]->Divide(2,2);
for(int i=0;i<4;i++){
  hnvtxnfvtxtrk[i] = (TH2F*)f->Get(Form("hnvtxnfvtxtrk_%d",i));
  hbbcsnvtx[i] = (TH2F*)f->Get(Form("hbbcsnvtx_%d",i));
  hbbcnnvtx[i] = (TH2F*)f->Get(Form("hbbcnvtx_%d",i));
  hbbcnvtx[i] = (TH2F*)f->Get(Form("hbbcnvtx_%d",i));
  hcluetaphi[i] = (TH2F*)f->Get(Form("hcluetaphi_%d",i));
c1[21]->cd(i+1);
c1[21]->SetLogz();
...
c1[21]->Print("fig/mb/hnvtxnfvtxtrk.png");
c1[25]->cd();
SetTitle(*hcntetaphi,"vtx #eta","vtx #phi","");
hcluetaphi[0]->Draw("colz");
c1[25]->Print("fig/mb/hcluetaphi_0.png");
...
SetRange(*hnvtx,0,1,400,1e10);
//SetTitle(*hnvtx,"#hits in cluster layer 1","normalized","");
SetTitle(*hnvtx,"#hits in cluster layer 1","# of events","");
SetStyle(*hnvtx,1.2,1,20,0,0);
hnvtx->Draw("P");
c1[22]->Print("fig/mb/hnvtx.png");
c1[23]->cd();
c1[23]->SetLogy();
//SetTitle(*hbbc,"bbc charge sum","normalized","");
SetTitle(*hbbc,"bbc charge sum","# of events","");
hbbc->Rebin(5);
//hbbc->Scale(1./hbbc->Integral());
//SetRange(*hbbc,0,1e-11,200,10);
SetRange(*hbbc,0,1,200,1e10);
SetStyle(*hbbc,1.2,1,20,0,0);
hbbc->Draw("P");
c1[23]->Print("fig/mb/hbbc.png");
c1[24]->cd();
c1[24]->SetLogz();
SetTitle(*hbbcnvtx[0],"bbc charge sum","#hits in cluster layer 1","");
TLine *l = new TLine(0,80,200,80);
l->SetLineStyle(2);
l->SetLineColor(2);
hbbcnvtx[0]->Draw("colz");
l->Draw("same");
c1[24]->Print("fig/mb/hbbcnvtx.png");
