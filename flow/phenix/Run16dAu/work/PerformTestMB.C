...
//cluster
  ...
  fvtxclusxy[ilayer] = NULL;
  fvtxclusetaphi[ilayer] = NULL;
  fvtxclusrphi[ilayer] = NULL;
  }
//mpc
...
//fvtx cluster
  for (int ilayer=0; ilayer<8; ilayer++) {
  sprintf(name,"fvtxclusxy_%d",ilayer); fvtxclusxy[ilayer] = new TH2F(name,name,200,-20,20,200,-20,20);
  sprintf(name,"fvtxclusetaphi_%d",ilayer); fvtxclusetaphi[ilayer] = new TH2F(name,name,200,-4,4,200,-4,4);
  sprintf(name,"fvtxclusrphi_%d",ilayer); fvtxclusrphi[ilayer] = new TH2F(name,name,300,0,30,200,-4,4);
  }
...
//  float fvtxy = eventfvtx_y;
 ...
 // Tracks 
  ...
  for(int iclus = 0; iclus < d_nFVTX_clus; iclus++){
    ...
    fvtxclusxy[istation]->Fill(fvtx_x,fvtx_y);
    fvtxclusetaphi[istation]->Fill(fvtx_eta,fvtx_phi);
    fvtxclusrphi[istation]->Fill(fvtx_r,fvtx_phi);
  }
  
  if ( mb_trigger == 0 )      continue;
...
//
for(int ilayer=0;ilayer<8;ilayer++){
  fvtxclusxy[ilayer]->Write();
  fvtxclusetaphi[ilayer]->Write();
  fvtxclusrphi[ilayer]->Write();
}
/*
//mpc
