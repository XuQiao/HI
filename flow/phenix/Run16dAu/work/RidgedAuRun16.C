...
RidgedAuRun16::RidgedAuRun16(std::vector<TString> input, std::vector<TString> input1, const char* output) :
  OutputFileName(output), InputFileName(input), InputFileName1(input1), ievent(0), jevent(0), nevent(0), mbuff(0), RunNumber(0)
{
...
//  float fvtxn_et[nbbc];
 ...
 //         }
  ...
  hcentbbcs->Fill(cent,bbc_s);
      ...
      // --- Run16dAu200                                                                                                    
   if ( RunNumber >= 454774 && RunNumber <= 455639 )    mbuff = 10;
      // --- Run16dAu62                                                                                                                     
   if ( RunNumber >= 455792 && RunNumber <= 456283 )    mbuff = 20;
      // --- Run16dAu20                                                                   
   if ( RunNumber >= 456652 && RunNumber <= 457298 )    mbuff = 40;
      // --- Run16dAu39                                                                        
   if ( RunNumber >= 457634 && RunNumber <= 458167 )    mbuff = 40;
  ...
  if(ntrack>0){//rebuff
	...
	trackbuff_eta[icent*vzbin*nbuff*ntrks+ivz*nbuff*ntrks+ibuff*ntrks+itrk_buff]=track_eta[itrk];
      ...
      if(itrk_buff>0){
        ...
        if(dtrack_buff[icent][ivz]<mbuff) dtrack_buff[icent][ivz]++;
        if(ntrack_buff[icent][ivz]>=mbuff) ntrack_buff[icent][ivz]=0;
      }
    }
  }//end of 4
  ...
  if(nbbcs>0){//rebuff
	...
	bbcsbuff_phi[icent*vzbin*nbuff*nbbc+ivz*nbuff*nbbc+ibuff*nbbc+ibbcs_buff]=bbcs_phi[ibbc];
      ...
      if(ibbcs_buff>0){
        ...
        if(dbbcs_buff[icent][ivz]<mbuff) dbbcs_buff[icent][ivz]++;
        if(nbbcs_buff[icent][ivz]>=mbuff) nbbcs_buff[icent][ivz]=0;
      }
    }
  }
  ...
  if(nfvtxs>0){//rebuff
    ...
    if(nfvtxs>0){//buffing lambda
      ...
      if(ibbcn_buff>0){
        ...
        if(dbbcn_buff[icent][ivz]<mbuff) dbbcn_buff[icent][ivz]++;
        if(nbbcn_buff[icent][ivz]>=mbuff) nbbcn_buff[icent][ivz]=0;
      }
    }
  }
