...
double fetchsub(TFile *fin, int icent, int isub, int ihar, int icorr, int ipt, int choosedcv, int iscorr){
    ...
    double wv2 = wc2>=0?(TMath::Power(1./4*wc2,1./6)):-9999;
    ...
    double wvcorr2 = wccorr2>=0?(TMath::Power(1./4*wccorr2,1./6)):-9999;
...
void fetchQ(){
  ...
  TFile *fin = TFile::Open("/gpfs/mnt/gpfs02/phenix/plhf/plhf1/xuq/phenix/flow/Run16dAu/work/200GeV/output_wQcumu.root","ReadOnly");
  TFile *fins[nsample];
  for(int isample=0;isample<nsample;isample++){
    fins[isample] = TFile::Open(Form("/gpfs/mnt/gpfs02/phenix/plhf/plhf1/xuq/phenix/flow/Run16dAu/work/200GeV/output_wQcumu_%d.root",isample),"ReadOnly");
  }
  TFile *fout = new TFile("outwP.root","recreate");
  ...
  for(int icent=0;icent<ncent;icent++){
    for(int ihar=0;ihar<nhar;ihar++){
      ...
      for(int isub=0;isub<nsub;isub++){
          cout<<"processing V icent = "<<icent<<" ihar = "<<ihar<<" isub = "<<isub<<endl;
        fdet<<"isub = "<<isub<<endl;
  ...
  for(int icent=0;icent<ncent;icent++){
    for(int ihar=0;ihar<nhar;ihar++){
        ...
        ofstream fvcorr(Form("Result/vcorr%d_cent%d.dat",n,icent));
      for(int isub=0;isub<nsub;isub++){
        cout<<"processing vpr icent = "<<icent<<" ihar = "<<ihar<<" isub = "<<isub<<endl;
        ...
        wv[icent][isub][ihar][2] = wc[icent][isub][ihar][2]>=0?(TMath::Power(1./4*wc[icent][isub][ihar][2],1./6)):-9999;
        ...
        wvcorr[icent][isub][ihar][2] = wccorr[icent][isub][ihar][2]>=0?(TMath::Power(1./4*wccorr[icent][isub][ihar][2],1/.6)):-9999;
