...
void fetchQ(){
  ...
  TFile *fin = TFile::Open("/gpfs/mnt/gpfs02/phenix/plhf/plhf1/xuq/phenix/flow/Run16dAu/work/200GeV/MCAna_mult300_fullacc.root","ReadOnly");
  TFile *fins[nsample];
  for(int isample=0;isample<nsample;isample++){
    fins[isample] = TFile::Open(Form("/gpfs/mnt/gpfs02/phenix/plhf/plhf1/xuq/phenix/flow/Run16dAu/work/200GeV/treeout/MCAna_mult300_fullacc_%d.root",isample),"ReadOnly");
  }
  ...
  for(int icent=0;icent<ncent;icent++){
    for(int ihar=0;ihar<nhar;ihar++){
      ...
      for(int isub=0;isub<nsub;isub++){
        ...
        fV<<" <derr> = " << wderr[icent][isub][ihar][icorr] <<" <cerr> = " << wcerr[icent][isub][ihar][icorr] <<" <verr> = " << wverr[icent][isub][ihar][icorr] << endl;
        fVcorr<<" <dcorr> = " << wdcorr[icent][isub][ihar][icorr] <<" <ccorr> = " << wccorr[icent][isub][ihar][icorr] <<" <vcorr> = " << wvcorr[icent][isub][ihar][icorr] << endl;
        fVcorr<<" <dcorrerr> = " << wdcorrerr[icent][isub][ihar][icorr] <<" <ccorrerr> = " << wccorrerr[icent][isub][ihar][icorr] <<" <vcorrerr> = " << wvcorrerr[icent][isub][ihar][icorr] << endl;
...
-12*wcos1[icent][isub][ihar]*wsin1[icent][isub][ihar]*
 (wsin1pr[icent][isub][ihar][ipt]*wcos1[icent][isub][ihar]-wcos1pr[icent][isub][ihar][ipt]*wsin1[icent][isub][ihar]);
        ...
        fv<<" <derr> = " << wdprerr[icent][isub][ihar][icorr][ipt] <<" <cerr> = " << wcprerr[icent][isub][ihar][icorr][ipt] <<" <verr> = " << wvprerr[icent][isub][ihar][icorr][ipt] << endl;
        ...
        fvcorr<<" <dcorrerr> = " << wdcorrprerr[icent][isub][ihar][icorr][ipt] <<" <ccorrerr> = " << wccorrprerr[icent][isub][ihar][icorr][ipt] <<" <vcorrerr> = " << wvcorrprerr[icent][isub][ihar][icorr][ipt] << endl;
        }
        fout->cd();
