...
void plotdphi(){
  ...
  //  for(int isub=0;isub<nsub;isub++){
        ...
        int ybinmin = h->GetXaxis()->FindBin(-TMath::Pi()/n);
        int ybinmax = h->GetXaxis()->FindBin(TMath::Pi()/n);
   ...
   // }
    ...
    h->Draw("PE");
    t.DrawLatex(0.18,0.85,"Run 16 d+Au 39 GeV, BBC-s charge > 50");// 0-5\% Central");
