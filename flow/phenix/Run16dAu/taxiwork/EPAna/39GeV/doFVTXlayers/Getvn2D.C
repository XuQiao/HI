...
float GetReso(int icent, int ihar, int isub, bool usingCNTEP){
    ...
    if(str1=="ABORT" || str2 == "ABORT") return -9999;
        ...
        continue;
         ...
         if(usingCNTEP){
            if(isub>=8 && isub<12) //do not store CNT-FVTX1-4LS correlation histograms, only use NOCNTEP
                return 0.2;
         ...
         TH1D* hEPR1temptemp = hvobstemp1->ProjectionY(Form("hEPR1%s%s_%d_%d","CNT",str1.Data(),icent,ihar),hvobstemp1->GetXaxis()->FindBin(0.4+1e-4),hvobstemp1->GetXaxis()->FindBin(2.0-1e-4));
         TH1D* hEPR2temptemp = hvobstemp2->ProjectionY(Form("hEPR2%s%s_%d_%d","CNT",str1.Data(),icent,ihar),hvobstemp1->GetXaxis()->FindBin(0.4+1e-4),hvobstemp1->GetXaxis()->FindBin(2.0-1e-4));
