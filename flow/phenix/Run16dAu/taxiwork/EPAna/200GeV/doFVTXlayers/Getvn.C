...
//          if(ihar!=1) continue;
       for(int isub=0;isub<nsub;isub++){
        ...
        std::cout<<"starting doing "<<str<<" v"<<n<<" analysis!"<<std::endl;
         fout1.open(Form("aResult/%s/res_%d%d_%d_%d_%s.dat",UseCNTEP.Data(),iangle1,iangle2,n,icent,str.Data())); //using str as event plane detector
         fout2.open(Form("aResult/%s/psi_%d%d_%d_%d_%s.dat",UseCNTEP.Data(),iangle1,iangle2,n,icent,str.Data())); //using str as event plane detector
        ...
        for(int iphi=0;iphi<nphi+1;iphi++){
         ...
         fout.open(Form("aResult/%s/v%d_%d%d_%d%s_%s.dat",UseCNTEP.Data(),n,iangle1,iangle2,icent,phistr.c_str(),str.Data())); //using str as event plane detector
         cout<<"open v"<<n<<" raw file"<<endl;
         foutraw.open(Form("aResult/%s/v%draw_%d%d_%d%s_%s.dat",UseCNTEP.Data(),n,iangle1,iangle2,icent,phistr.c_str(),str.Data())); //using str as event plane detector
         if(iphi<nphi){
        for(int irun=0;irun<nrun;irun++){
    ...
    if(str1=="ABORT" || str2 == "ABORT") return -9999;
        ...
        if(hEPR1->GetMean()*hEPR3->GetMean()/hEPR2->GetMean()>0){
            ...
            return reso;
             ...
             cout<<hEPR1->Integral(xbinmin,xbinmax)/hEPR12->Integral(xbinmin,xbinmax)<<endl;
             cout<<hEPR3->GetMean()<<endl;
             cout<<hEPR2->Integral(xbinmin,xbinmax)/hEPR22->Integral(xbinmin,xbinmax)<<endl;
