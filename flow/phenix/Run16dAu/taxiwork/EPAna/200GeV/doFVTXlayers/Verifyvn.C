...
//leg->AddEntry(grold_fvtx1s,"PPG161 Run8");
leg->AddEntry(gr[5][idire][iCNTEP],Form("Run16 FVTX -3.0<#eta<-1.0 as EP"));
leg->AddEntry(gr[4][idire][iCNTEP],Form("Run16 BBCS as EP"));
...
h->SetMinimum(0);
h->SetMaximum(2);
SetTitle(h,"","p_{T}","v_{2} ratio Run16/Run8");
h->DrawCopy();
TGraphErrors *grr = (TGraphErrors*)DivideTwoGraphs(gr[5][idire][iCNTEP],grold_fvtx1s);
SetStyle(*grr,1.2,color[idire+3*iCNTEP+5+3],style[5]);
grr->Draw("Psame");

TGraphErrors *grr = (TGraphErrors*)DivideTwoGraphs(gr[4][idire][iCNTEP],grold_fvtx1s);
