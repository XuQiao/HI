...
void plotdphiFORMEETING(){
  ...
  //  for(int isub=0;isub<nsub;isub++){
   ...
   // }
    ...
    if(n==3)
    h->GetYaxis()->SetRangeUser(0.996, 1.006);
    if(n==2)
    //h->GetYaxis()->SetRangeUser(0.85, 1.15);
    h->GetYaxis()->SetRangeUser(0.96, 1.055);
    //h->GetYaxis()->SetRangeUser(0.85, 1.15);
    ...
    h->GetYaxis()->SetTitle(Form("dN / d(#phi - #Psi_{%d})  A.U.",n));
    h->Draw("PE");
    t.SetTextSize(0.046);
    t.DrawLatex(0.12,0.85,"Run 16 d+Au 200 GeV, 0-5\% Central");
    t.SetTextSize(0.04);
    t.DrawLatex(0.12,0.75,"|#eta| < 0.35");
    t.DrawLatex(0.12,0.70,Form("%.1f < p_{T} < %.1f GeV/c",ptmin,ptmax));
    t.SetTextSize(0.055);
    t.SetTextColor(2);
    t.DrawLatex(0.65,0.72,Form("n = %d",n));
    /*
     if(n==2)
    ...
    leg->Draw("same");
     */
    c1->Print(Form("dphinFORMEETING_%d_%d_%d.png",n,icent,isub));
}
