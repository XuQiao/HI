...
void Verifyvn(){
    ...
    int icent = 0;
    int n = 2;
    int color[18] = {1,2,4,5,7,8,1,2,4,5,7,8,1,2,4,5,7,8};
    ...
    for(int iCNTEP=0;iCNTEP<2;iCNTEP++){
        ...
        SetStyle(*gr[isub][idire][iCNTEP], 1.2, color[idire+3*iCNTEP+isub],style[isub]);
...
leg->AddEntry(gr[5][idire][iCNTEP],Form("Run16 20GeV 0-5%% FVTX -3.0<#eta<-1.0 as EP"),"P");
leg->AddEntry(gr[4][idire][iCNTEP],Form("Run16 20GeV 0-5\% BBCS as EP"),"P");
...
SetStyle(*grr,1.2,color[idire+3*iCNTEP+isub],style[5]);
grr->Draw("Psame");

TGraphErrors *grr = (TGraphErrors*)DivideTwoGraphs(gr[6][idire][iCNTEP],gr_fvtx1s);
SetStyle(*grr,1.2,color[idire+3*iCNTEP+isub],style[4]);
grr->Draw("Psame");
