...
void plotdphi(){
    ...
    TFile *fin = TFile::Open(Form("merged.root"));
  //  for(int isub=0;isub<nsub;isub++){
        TH2F* hpt_0 = (TH2F*)fin->Get(Form("vn%s_0_0_%d_%d_%d",choosesub(isub).Data(),icent,ihar,0));
        TH2F* hpt_1 = (TH2F*)fin->Get(Form("vn%s_0_0_%d_%d_%d",choosesub(isub).Data(),icent,ihar,1));
        TH2F* hpt = (TH2F*)hpt_0->Clone(Form("vn%s_0_0_%d_%d",choosesub(isub).Data(),icent,ihar));
        hpt->Add(hpt_1);
   ...
   // }
    ...
    h->Draw("PE");
    t.DrawLatex(0.18,0.85,"Run 16 d+Au 20 GeV, BBC-s charge > 40");// 0-5\% Central");
    t.DrawLatex(0.18,0.75,"|#eta| < 0.35");
    t.DrawLatex(0.18,0.70,Form("%.1f < p_{T} < %.1f GeV/c",ptmin,ptmax));
    t.SetTextSize(0.04);
    t.SetTextColor(2);
    if(n==2)
