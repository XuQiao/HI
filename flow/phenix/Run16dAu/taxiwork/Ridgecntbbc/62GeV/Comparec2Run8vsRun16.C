...
//  ifstream finc("c1_c2_pt_south_bbc_00_05_dAu_ppg161.dat");
  ifstream finc("../200GeV/c1_c2_central_ptfiner_south.dat");//pp x Et
  ifstream finp("c1_c2_central_ptfiner_south.dat");//pp x Et
 int n = 2;
  float pt[10], ept[10], spt[10];

  float c2_c[10], ec2_c[10], sc2_c[10];
  float c2_p[10], ec2_p[10], sc2_p[10];
  float tmp;
  
  float ratio[10], eratio[10], sratio[10];
  ...
  TGraphErrors * grp = new TGraphErrors(9, pt, c2_p, ept, ec2_p);
 ...
 // sgrc->Draw("PE2");
  ...
  TLegend *leg1 = new TLegend(0.15,0.64,0.35,0.88);
  ...
  leg1->AddEntry(grc,Form("A = c_{%d}^{dAu Run16 200GeV}; cent:0-5%%",n),"P");
  leg1->AddEntry(grp,Form("B = c_{%d}^{dAu Run16 62GeV}; BBCs >= 50",n),"P");
  leg1->Draw();
  ...
  t2->SetTextSize(0.10);
 // t2->Draw();
  c1->Print(Form("Compc%dRun8vsRun16.png",n));
}
