...
void plotcorr_5p_pt(){
  ...
  gStyle->SetErrorX(0);
c1 = new TCanvas("c1"," ",1400,340);
makeMultiPanelCanvas(c1,5,1,0,0,0.25,0.2,0.03);
TString dire = "north";
//TString dire = "south";
...
if(dire=="north"){
float Mbbc[ncent] = {8.58,7.51,6.71,5.58,3.52,2.80};
}
TFile *f=TFile::Open("../../../work/39GeV/output_Ridge.root");
...
for(int ipt=0; ipt<5; ipt++){
  int ipt_add;
	if(ipt==0){
for(int ipt_add=1;ipt_add<3;ipt_add++){
...
for(int ipt_add=3;ipt_add<6;ipt_add++){
kforebbcw_centIn[ipt]->Add(kforebbcw_centIn[ipt_add]);
hforebbcw_centIn[ipt]->Add(hforebbcw_centIn[ipt_add]);
kbackbbcw2_centIn[ipt]->Add(kbackbbcw2_centIn[ipt_add]);
}
...
hforebbcw_centIn[ipt]->Reset("M");
kbackbbcw2_centIn[ipt]->Reset("M");
for(int ipt_add=6;ipt_add<9;ipt_add++){
kforebbcw_centIn[ipt]->Add(kforebbcw_centIn[ipt_add]);
hforebbcw_centIn[ipt]->Add(hforebbcw_centIn[ipt_add]);
kbackbbcw2_centIn[ipt]->Add(kbackbbcw2_centIn[ipt_add]);
}
...
hforebbcw_centIn[ipt]->Reset("M");
kbackbbcw2_centIn[ipt]->Reset("M");
for(int ipt_add=9;ipt_add<12;ipt_add++){
kforebbcw_centIn[ipt]->Add(kforebbcw_centIn[ipt_add]);
hforebbcw_centIn[ipt]->Add(hforebbcw_centIn[ipt_add]);
kbackbbcw2_centIn[ipt]->Add(kbackbbcw2_centIn[ipt_add]);
}
...
hforebbcw_centIn[ipt]->Reset("M");
kbackbbcw2_centIn[ipt]->Reset("M");
for(int ipt_add=12;ipt_add<15;ipt_add++){
kforebbcw_centIn[ipt]->Add(kforebbcw_centIn[ipt_add]);
hforebbcw_centIn[ipt]->Add(hforebbcw_centIn[ipt_add]);
kbackbbcw2_centIn[ipt]->Add(kbackbbcw2_centIn[ipt_add]);
}
...
//  float ymin = 0.968;//hpp[ipt]->GetMinimum()*0.9;
  float y = TMath::Max(TMath::Abs(hpp[4]->GetBinContent(hpp[4]->GetMaximumBin())-1),TMath::Abs(hpp[4]->GetBinContent(hpp[4]->GetMinimumBin())-1));
  float ymax = 1 + y * 1.02;
  float ymin = 1 - y * 0.98;
...
//  m0->Draw();
  ...
  if(ipt==0){
  TLatex *t=new TLatex(-0.4,0.88*(ymax-ymin)+ymin, Form("Run16 d+Au 39GeV"));
  t->SetTextSize(0.06);
  t->Draw();
  TLatex *t=new TLatex(1.0,0.78*(ymax-ymin)+ymin, Form("%.1f < p_{T}^{trig} < %.1f (GeV/c)",ptbin[ipt],ptbin[ipt+1]));
  }
  else
  TLatex *t=new TLatex(1.0,0.78*(ymax-ymin)+ymin, Form("%.1f < p_{T}^{trig} < %.1f (GeV/c)",ptbin[ipt],ptbin[ipt+1]));
 // TLatex *t=new TLatex(-0.1,0.88*(ymax-ymin)+ymin, Form("p+p %.1f < p_{T} < %.1f (GeV/c)",ptbin[ipt],ptbin[ipt+1]));
  ...
  if(ipt==0)t->Draw();
  if(dire=="north")
  TLatex *t=new TLatex(-1.2,0.28*(ymax-ymin)+ymin, Form("<N_{FVTXs} = %.2f>",Mbbc[icent_a]));
  if(dire=="south")
  TLatex *t=new TLatex(-1.2,0.28*(ymax-ymin)+ymin, Form("<Q_{BBCs} = %.2f>",Mbbc[icent_a]));
  t->SetTextSize(0.06);
  if(ipt==0)t->Draw();
...
/*
  ...
  ty3->Draw();
*/
  c1->Print(Form("fig/ridge5p_pt_%s_%.1f_%.1f.gif",dire.Data(),centmin,centmax));
  c1->Print(Form("fig/ridge5p_pt_%s_%.1f_%.1f.png",dire.Data(),centmin,centmax));
  c1->Print(Form("fig/ridge5p_pt_%s_%.1f_%.1f.pdf",dire.Data(),centmin,centmax));
}
