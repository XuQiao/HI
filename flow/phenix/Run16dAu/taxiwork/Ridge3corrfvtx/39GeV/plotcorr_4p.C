...
*/
  ...
  float ymax = 1.02;//hpp[icent]->GetMaximum()*1.1;
  float ymin = 0.98;//hpp[icent]->GetMinimum()*0.9;
...
TLegend *leg1 = new TLegend(0.22,0.22,0.82,0.32);
 ...
 // t->Draw();
  ...
  TLatex *t=new TLatex(-0.9,0.88*(ymax-ymin)+ymin, Form("d+Au 39GeV %.f - %.f%%",centbin[icent],centbin[icent+1]));
  }
  else{
