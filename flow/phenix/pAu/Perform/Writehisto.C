void Writehisto(){
    ...
    if(trigtype.Contains("fvtx"))  {
        ...
        name2 = "../filelistfvtx3.dat"; //centrality calibrated
    }
    else if(trigtype.Contains("mb")){
        name = "../filelistmb.dat";
        name1 = "../filelistmb2.dat"; //event calibrated
        name2 = "../filelistmb3.dat"; //centrality calibrated
    }
    else exit();
    for(int i=start;i<end;i++){
        cout << i << endl;
        Perform *pl = new Perform(readline(Form("%s",name.Data()),i),readline(Form("%s",name1.Data()),i), readline(Form("%s",name2.Data()),i),Form("/scratch/xuq7/phenix/pAu/Perform/Ana%s_%d.root",trigtype.Data(),i));
        pl->Init();
        pl->process_event();
        pl->End();
        delete pl;
