...
bbcau_buff bbcaubuff_pt(nbuffbbcs);
...
Ridgecntcnt::Ridgecntcnt(TString input, TString output):
...
int Ridgecntcnt::process_event()
{
  int nEvent = 100;//tree->GetEntries();
  cout << nEvent << endl;
...
//global
  ...
  float bbcde_eta[nbbcs];
        ...
        if((fabs(KF[iparticle]) == 211 || fabs(KF[iparticle]) == 213 || fabs(KF[iparticle]) == 321 || fabs(KF[iparticle]) == 323 ||fabs(KF[iparticle]) == 2212)) 
            nch++;
    }
        //if((fabs(KF[iparticle]) == 211 || fabs(KF[iparticle]) == 321 || fabs(KF[iparticle]) == 2212)) 
    }
    int chbin[centbin+1] = {0,1,2,3,4,5,7,10,14,18,40};
  ...
  for(int itrk=0; itrk<ntrk; itrk++){
    ...
    for(int ipmt=0; ipmt<nbbcau; ipmt++){
      if(!(bbcau_pt[ipmt]>1 && bbcau_pt[ipmt]<2)) continue;
      ...
      if(fabs(deta)<1e-4 && fabs(dphi)<1e-4) continue; //remove auto-correction
  ...
  if(dtrack_buff[icent][ivz]>0){
	...
	for(int ipmt=0; ipmt<nbbcau; ipmt++){
          if(!(bbcau_pt[ipmt]>1 && bbcau_pt[ipmt]<2)) continue; //asso pt range
          float val = bbcau_et[ipmt];
          float bbc_phi = bbcau_phi[ipmt];
          float bbc_eta = bbcau_eta[ipmt];
  ...
  for(int itrk=0; itrk<ntrk; itrk++){
    ...
    if(dbbcau_buff[icent][ivz]>0){
      for(int ibuff=0;ibuff<dbbcau_buff[icent][ivz];ibuff++){
        ...
        if(!(bbcaubuff_pt[icent*vzbin*nbuff*nbbcs+ivz*nbuff*nbbcs+ibuff*nbbcs+ipmt_buff]>1 && bbcaubuff_pt[icent*vzbin*nbuff*nbbcs+ivz*nbuff*nbbcs+ibuff*nbbcs+ipmt_buff]<2)) continue;
  ...
  if(nbbcau>0){//rebuff
	...
	bbcaubuff_pt[icent*vzbin*nbuff*nbbcs+ivz*nbuff*nbbcs+ibuff*nbbcs+ibbc_buff]=bbcau_pt[ibbc];
