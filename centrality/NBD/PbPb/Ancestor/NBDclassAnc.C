...
public:
	...
	TVectorD NcollRMS, NpartRMS, BRMS;
	...
	void initx(double xminv, double xmaxv){
		...
		method.ResizeTo(1);
                ...
                NpartRMS.ResizeTo(N); NcollRMS.ResizeTo(N);BRMS.ResizeTo(N);
		...
		//if(Ev%100000==0) cout<<"Have run "<<Ev<<" events"<<endl;
	        	...
	        	if(Mult>kpoint[i]&&Mult<=kpoint[i+1]){
        	        	NcollAver[i] += Ncoll;
                	        NpartAver[i] += Npart;
                        	BAver[i] += B;
                                NcollRMS[i] += Ncoll * Ncoll;
                	        NpartRMS[i] += Npart * Npart;
                        	BRMS[i] += B * B;
			   	PartEvent[i]++;
                     	 	TotalEvent++;
				hNpart[i]->Fill(Npart);
        ...
        for(i=0; i<N; i++){
        	if(PartEvent[i]){
                	NcollAver[i]/=PartEvent[i];
                        ...
                        BAver[i]/=PartEvent[i];
                        NcollRMS[i]=TMath::Sqrt(NcollRMS[i]/PartEvent[i]-NcollAver[i]*NcollAver[i]);
                        NpartRMS[i]=TMath::Sqrt(NpartRMS[i]/PartEvent[i]-NpartAver[i]*NpartAver[i]);
                        BRMS[i]=TMath::Sqrt(BRMS[i]/PartEvent[i]-BAver[i]*BAver[i]);
