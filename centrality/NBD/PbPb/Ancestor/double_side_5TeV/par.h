//const double bin[]={0,12.61,18.99,23.23,29.50,150};	//G1.root
double bin[]={
1.0, 0.995, 0.99, 0.985, 0.98, 0.975, 0.97, 0.965, 0.96, 0.955, 0.95, 0.945, 0.94, 0.935, 0.93, 0.925, 0.92, 0.915, 0.91, 0.905, 0.9, 0.895, 0.89, 0.885, 0.88, 0.875, 0.87, 0.865, 0.86, 0.855, 0.85, 0.845, 0.84, 0.835, 0.83, 0.825, 0.82, 0.815, 0.81, 0.805, 0.8, 0.795, 0.79, 0.785, 0.78, 0.775, 0.77, 0.765, 0.76, 0.755, 0.75, 0.745, 0.74, 0.735, 0.73, 0.725, 0.72, 0.715, 0.71, 0.705, 0.7, 0.695, 0.69, 0.685, 0.68, 0.675, 0.67, 0.665, 0.66, 0.655, 0.65, 0.645, 0.64, 0.635, 0.63, 0.625, 0.62, 0.615, 0.61, 0.605, 0.6, 0.595, 0.59, 0.585, 0.58, 0.575, 0.57, 0.565, 0.56, 0.555, 0.55, 0.545, 0.54, 0.535, 0.53, 0.525, 0.52, 0.515, 0.51, 0.505, 0.5, 0.495, 0.49, 0.485, 0.48, 0.475, 0.47, 0.465, 0.46, 0.455, 0.45, 0.445, 0.44, 0.435, 0.43, 0.425, 0.42, 0.415, 0.41, 0.405, 0.4, 0.395, 0.39, 0.385, 0.38, 0.375, 0.37, 0.365, 0.36, 0.355, 0.35, 0.345, 0.34, 0.335, 0.33, 0.325, 0.32, 0.315, 0.31, 0.305, 0.3, 0.295, 0.29, 0.285, 0.28, 0.275, 0.27, 0.265, 0.26, 0.255, 0.25, 0.245, 0.24, 0.235, 0.23, 0.225, 0.22, 0.215, 0.21, 0.205, 0.2, 0.195, 0.19, 0.185, 0.18, 0.175, 0.17, 0.165, 0.16, 0.155, 0.15, 0.145, 0.14, 0.135, 0.13, 0.125, 0.12, 0.115, 0.11, 0.105, 0.1, 0.095, 0.09, 0.085, 0.08, 0.075, 0.07, 0.065, 0.06, 0.055, 0.05, 0.045, 0.04, 0.035, 0.03, 0.025, 0.02, 0.015, 0.01, 0.005,0.0};	//G0.root
//const double bin[]={1.0,0};	//G2.root
int N=(int)(sizeof(bin)/sizeof(double));
int method=0; //method=0: use bin; method=1: use kpoint

TString datafile="/scratch/xuq7/Data/PbPbHistEta4.root";

TString histoname="hHFEnergy4";

double binshift = 20;

struct para1 var1={40,4000,	1,3,0.2,    1.0,3.0,0.2, 0.5,1.5,0.2};
struct para2 bestlist1[nGlau+2]={
//    {2,2,0.9},{2,2,0.9},{2,2,0.9},{2,2,0.9},{2,2,0.9},{2,2,0.9},{2,2,0.9},{2,2,0.9},{2,2,0.9},{2,2,0.9},{2,2,0.9}  //coarse bin, first pass
    {1.2,2.6,0.7},{1.2,2.8,0.7},{1.2,2.8,0.7},{1.2,2.8,0.7},{1.2,2.2,0.7},{2,1.6,0.9},{1.2,2.2,0.7},{1.2,1.6,0.7},{2,2.2,0.9},{1.2,2.6,0.7},{1.2,2.6,0.7}
};

	
struct para1 var2={15,70,	1.00,2.01,0.50,	2.01,5.01,0.2, 0.9,1.1,0.05};   //Gribov Omega = 0.55
struct para2 bestlist2[nGlau+2]={
    {1.5,3.52},{1.5,3.916},{1.503,3.008},{1.5,3.52},{1.5,3.52},{1.402,2.604},{1.5,2.42},{1.601,3.117},{1.4,3.913},{1.5,3.12},{1.5,3.52}
};


struct para1 var3={15,70,	1.00,2.01,0.50,	2.01,5.01,0.2, 0.9,1.1,0.05};   //Gribov Omega = 1.01

struct para2 bestlist3[nGlau+2]={
    {1.3,2.419},{1.3,2.918},{1.4,3.919},{1.3,2.419},{1.3,2.419},{1.3,3.42},{1.4,3.418},{1.5,3.92},{1.2,2.22},{1.4,3.82},{1.3,2.419}
};


TString outG = Form("G%d.root",method);
