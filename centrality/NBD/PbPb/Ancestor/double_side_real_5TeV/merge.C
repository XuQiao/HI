...
void merge(){
	...
	TVectorD *NcollRMS[nGlau+2], *NpartRMS[nGlau+2], *BRMS[nGlau+2];
	...
	for(int sth=0;sth<1;sth++){
		...
		centbin_ = (TVectorD*)f->Get(Form("%s/G0/centbin_",dirname.Data()));
        	for(int i=0;i<N-1;i++){
				...
				NcollRMS[iGlau] = (TVectorD*)f->Get(Form("%s/%s/NcollRMS",dirname.Data(),name.Data()));
				NpartRMS[iGlau] = (TVectorD*)f->Get(Form("%s/%s/NpartRMS",dirname.Data(),name.Data()));
				BRMS[iGlau] = (TVectorD*)f->Get(Form("%s/%s/BRMS",dirname.Data(),name.Data()));
                                if((*NcollAver)[0]!=0){
...
//<<(*NpartAver[0])[i]<<"\t"<<Nparterr[i]<<"\t\t"<<(*BAver[0])[i]<<"\t"<<Berr[i]<<endl;
	...
	std::vector<double> xbin(N-1);
        ...
        TGraphErrors *gr;
       		...
       		double* aNcollRMS  = NcollRMS[0]->GetMatrixArray();
        	double* aNcollerr  = Ncollerr.GetMatrixArray();
        	double* aNpartAver  = NpartAver[0]->GetMatrixArray();
        	double* aNpartRMS  = NpartRMS[0]->GetMatrixArray();
        	double* aNparterr  = Nparterr.GetMatrixArray();
        	double* aBAver  = BAver[0]->GetMatrixArray();
        	double* aBRMS  = BRMS[0]->GetMatrixArray();
        	double* aBerr = Berr.GetMatrixArray();
	TString op;
	for(int option=0; option<3; option++){
        	if(option==0){   gr = new TGraphErrors(N-1,axbin,aNcollAver,0,aNcollRMS); op="Ncoll_graph";}
        	else if(option==1){      gr = new TGraphErrors(N-1,axbin,aNpartAver,0,aNpartRMS);	op="Npart_graph";}
        	else    {gr = new TGraphErrors(N-1,axbin,aBAver,0,aBRMS);	op="B_graph";}
