...
void Drawmethodcomp(){ 
 ...
 ifstream fsmear("smear.dat");
 double centDilbin[nDil];
 double centDilbinerr[nDil];
 double tmp;
 double NcollDil[nDil];
 double NpartDil[nDil];
 double NcollDilerr[nDil];
 double NpartDilerr[nDil];
 N=N-1;
for(int i=0;i<nDil;i++){
//  centDilbin[i]=N-(0.29+i+N-1-nDil);
  centDilbin[i] = N-(i+0.5)*N/nDil;  
  centDilbinerr[i] = 0.5*N/nDil;  
  fsmear>>tmp>>tmp>>NpartDil[i]>>NcollDil[i];
  cout<<NpartDil[i]<<" "<<NcollDil[i]<<endl;
  NcollDilerr[i]=0;
  NpartDilerr[i]=0;
...
// const double NcollDil[nDil] = {8.01,38.86,86.85,175.76,326.06,563.21,926.79,1484.49};
 ...
 TGraphErrors *graphNpartDil = new TGraphErrors(nDil,centDilbin,NpartDil,centDilbinerr,NpartDilerr);
 TGraphErrors *graphNcollDil = new TGraphErrors(nDil,centDilbin,NcollDil,centDilbinerr,NcollDilerr);
 ...
 hist->GetXaxis()->SetNdivisions(N);
if(method==0)
 hist->SetXTitle("Centrality");
else
 hist->SetXTitle("HF #Sigma E_{T}");
...
//	TGraphErrors* Gri101_graph = (TGraphErrors*)f->Get(Form("Gri101/%s_graph",str.Data()));
	...
	TFile *f1=TFile::Open(Form("../../OneComp/double_side_real_5TeV/%s",outG.Data()));
...
//    leg0->AddEntry(Gri101_graph,"Gribov #Omega=1.01","p");
 if(str=="Npart")   leg0->AddEntry(graphNpartDil,"Npart from smearing method","p");
 if(str=="Ncoll")   leg0->AddEntry(graphNcollDil,"Ncoll from smearing method","p");
	leg0->Draw();	
c1->SaveAs(Form("%sGriComp.png",str.Data()));
c1->SaveAs(Form("%sGriComp.pdf",str.Data()));
...
for(int ip = 0;ip<fdvf1->GetN();ip++){
    ...
    double eyNcoll = graphNcollDil->GetEY()[N-1-ip];
    double yNcoll = graphNcollDil->GetY()[N-1-ip];
    double eyNpart = graphNpartDil->GetEY()[N-1-ip];
    double yNpart = graphNpartDil->GetY()[N-1-ip];
    if(y1!=0 && y!=0){
...
TLegend *leg1 = new TLegend(0.38,0.72,0.75,0.86);
    ...
    leg1->AddEntry(f1dvDil,"One Component fitting/smearing fitting","p");
    leg1->AddEntry(fdvDil,"Two Component fitting/smearing fitting","p");
    leg1->AddEntry(Dilsys,"smearing fitting systematics","f");
  //  leg1->AddEntry(Dilsys2,"RunI fitting ratio systematics","f");
   ...
   leg1->Draw();
    TFile *fout = new TFile("Table_GlauberNBD_v0.root","Update");
    fout->cd();
    f1dvDil->Write(Form("%s_f1dvDil",str.Data()),TObject::kOverwrite);
    fdvDil->Write(Form("%s_fdvDil",str.Data()),TObject::kOverwrite);
    Dilsys->Write(Form("%s_Dilsys",str.Data()),TObject::kOverwrite);
    fout->Close();
