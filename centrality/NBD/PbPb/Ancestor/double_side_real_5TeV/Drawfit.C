...
void Drawfit(){
	...
	TH1D *histo_obs = (TH1D*)fdata->Get(histoname->GetString());
        ...
        cout<<histo_obs->GetBinContent(10)<<" "<<histo_obs->GetBinContent(2)<<endl;
	...
	c1->SetLogy();
        ...
        histo_obs_norm->GetXaxis()->SetRangeUser(0,6000);
	...
	histo_obs_norm->Draw("Psame");
        ...
        fout->cd();
        histo_exp_norm->Write("histo_exp_norm",TObject::kOverwrite);
        histo_obs_norm->Write("histo_obs_norm",TObject::kOverwrite);
        fout->Close();
