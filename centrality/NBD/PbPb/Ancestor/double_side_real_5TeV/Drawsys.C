...
void Drawsys(){ 
 TCanvas *c1 = new TCanvas("c1","c1",1,1,550,460);
  ...
  gStyle->SetOptStat(0);
  c1->SetTicks(-1);
TString Var[nGlau+2]={"standard","Depth0536","Depth0556","dmin00","dmin08","Radius656","Radius668","sNN65mb","sNN75mb",Form("bin-%.f",binshift),Form("bin+%.f",binshift)};
...
if(method==0)
 ...
 hist->Draw();
	...
	TGraph *graph[nGlau+2];
        ...
        if(str=="Npart")
        TVectorD* NcollAver0 = (TVectorD*)f->Get(Form("%s/G0/NpartAver",dirname.Data()));
	...
	for(int iGlau=0;iGlau<nGlau+2;iGlau++){
   		if(iGlau==0)
                ...
                if(str=="Npart")
               	TVectorD* NcollAver = (TVectorD*)f->Get(Form("%s/%s/NpartAver",dirname.Data(),name.Data()));
		ratio.clear();
		for(int i=0;i<N;i++){
        	xbin[i]=0.29+i;
...
for(int i=0;i<N;i++)
    ...
    TLatex *tex1= new TLatex(0.6,1.12,"CMS Preliminary PbPb #sqrt{s_{NN}} = 5 TeV");
    tex1->SetTextColor(1);
    tex1->SetTextFont(42);
...
TLegend *leg2 = new TLegend(0.20,0.15,0.35,0.40);
  ...
  for(int iGlau=0; iGlau<nGlau+2; iGlau++){
   ...
   leg2->Draw();	
    ...
    fout->cd();
    for(int iGlau=0;iGlau<nGlau+2;iGlau++){
        graph[iGlau]->Write(Form("%s_%s",str.Data(),Var[iGlau].Data()),TObject::kOverwrite);
    }
    fout->Close();
    c1->SaveAs(Form("%s%ssys.png",str.Data(),dirname.Data()));
}
